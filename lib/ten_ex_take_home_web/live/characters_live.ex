defmodule TenExTakeHomeWeb.CharactersLive do
  ...
  def handle_event(
        "next-characters",
        _,
        %{assigns: %{page: page, results_per_page: results_per_page}} = socket
      ) do
  ...
  def handle_event(
        ...
        %{"results_per_page" => new_results_per_page} = _payload,
        %{assigns: %{page: page}} = socket
      ) do
    {:ok, characters} =
      MarvelApi.get_all_characters(page, String.to_integer(new_results_per_page))
